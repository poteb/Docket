...
- **Execution is always delegated to a sub-agent**, so the main conversation stays free for the user to queue more tasks. When the user says "Proceed", dispatch a sub-agent to take the **earliest** `todo` task (smallest filename = oldest-created). "Proceed with `<slug>`" dispatches for the task whose slug matches.
